...
--kl_anneal_cyclical \
--log_dir ./logs/lp_epoch300_cycle
--cuda
...
def train(x, cond, modules, optimizer, kl_anneal, args):
    ...
    # not sure [bs, 12, 3, 64, 64] ->  [12, bs, 3, 64, 64]
    # torch.permute(input tensor, The desired ordering of dimensions)
    ...
    for i in range(1, args.n_past + args.n_future):
        # h5
        h_target = h_seq[i][0]
        # skip connections go between frame t and frame t+t 
        if args.last_frame_skip or i < args.n_past:	
            # h5, [h1, h2, h3, h4]
            h, skip = h_seq[i-1]
        # skip connections go between frame t and last ground truth frame
        else:
            # h5
            h = h_seq[i-1][0]
        ...
        # concatenates over the first dimension (column)
    ...
    # Lθ,φ,ψ(x1:T) = Σ[Eqφ(z1:t|x1:t)log pθ(xt|x1:t−1,z1:t)−β DKL(qφ(zt|x1:t)||pψ(zt|x1:t−1))]
    loss = mse + kld * beta
...
def main():
    ...
    if args.model_dir != '':
        ...
        # frame_predictor : args.g_dim + args.z_dim + 7
    ...
    # train 43863 sequences, batch size = 12, 43863 / 12 = ...3584
    # 12 sequences x 30 frames 
